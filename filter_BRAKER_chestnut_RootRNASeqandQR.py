...
##----------------------------------------------------------
## Get BLAST  to QR
bfile = "../filter_by_QR_support/reciprocal_best_hits.txt"
with open(bfile,'r') as b_in:
	for line in b_in:
		...
		genes2evi[stripped_match.group(1)] = 'QR'
b_in.close
...
qr = sum(1 for x in genes2evi.values() if x == 'QR')
print("Genes with QR evidence only: "+ str(qr))

rna = sum(1 for x in genes2evi.values() if x == 'RNA')
print("Genes with RNA evidence only: "+ str(rna))

both = sum(1 for x in genes2evi.values() if x == 'QR,RNA')
print("Genes with QR and RNA evidence: "+ str(both))
...
gfile = "../../4_BRAKER_rerun/braker/chestnut082218_RNASeqRootandQrobur/BRAKER_RootRNASeqandQR_augustus.hints.gff"
